...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        pre_deltas = self.dynamics_models[i](norm_obs_acs)
        norm_deltas = ((next_obs-obs) - self.obs_delta_mean) / (self.obs_delta_std + 1e-8)

        loss = self.loss_fn(norm_deltas, pre_deltas)
    ...
    def get_dynamics_predictions(
        ...
        norm_obs_acs = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        pre_deltas = self.dynamics_models[i](norm_obs_acs)
        pred_next_obs = obs + pre_deltas * (self.obs_delta_std + 1e-8) + self.obs_delta_mean
